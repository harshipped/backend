...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    const accessToken = user.generateAccessToken();
    // console.log("Access token:", accessToken); // check if token generates

    const refreshToken = user.generateRefreshToken();
    // console.log("Refresh token:", refreshToken); // check if token generates
    ...
    // console.log("Error:", error); // see the ACTUAL error
    throw new ApiError(
      500,
      "Something went wrong while generating access or refresh token"
    );
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      ...
      .json(200, { accessToken, newRefreshToken }, "Access tokken refreshed");
  } catch (error) {
